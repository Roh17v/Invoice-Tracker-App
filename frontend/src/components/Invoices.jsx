...
const Invoices = () => {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${HOST}${GET_ALL_INVOICES_ADMIN}?${queryParams.toString()}`,
        { withCredentials: true }
      );
  ...
  const handleInvoiceAction = async (invoiceId, action, data = {}) => {
    try {
      ...
      if (action === "paid") setShowPaidModal(false);
    } catch (err) {
      console.error(
  ...
  const handleOpenPaidModal = (invoice) => {
    setSelectedInvoice(invoice);
    setActionNote("");
  ...
  const handlePaid = (invoiceId) => {
    handleInvoiceAction(invoiceId, "paid", {
      status: "paid",
      note: actionNote.trim() || "Marked as paid by admin",
    });
  };

  // Open history/file modals
      ...
      <div className="min-h-screen bg-gray-50 py-12 mt-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                          ...
                          <div className="flex flex-col space-y-2">
                            {(currentUser.role === "admin" ||
                              ...
                              <>
                                ...
                                <button
                                  onClick={() => handleOpenRejectModal(invoice)}
                                ...
                                {currentUser.role === "admin" && (
                                  <button
                                    onClick={() => handleOpenPaidModal(invoice)}
                                    className="text-blue-600 hover:text-blue-800 flex items-center text-sm"
                                  >
          ...
          {showPaidModal && selectedInvoice && (
            <Transition appear show={showPaidModal} as={Fragment}>
              <Dialog
                as="div"
                className="relative z-50"
                  ...
                  <div className="flex min-h-full items-center justify-center p-4">
                    ...
                    >
                      <DialogPanel className="w-full max-w-md bg-white rounded-lg p-6">
                        ...
                        >
                          Mark Invoice #
                          {selectedInvoice.number || selectedInvoice._id} as
                          Paid
                        </DialogTitle>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Note (Optional)
                        ...
                        <div className="flex justify-end space-x-2">
                          ...
                          <button
                            onClick={() => handlePaid(selectedInvoice._id)}
                            className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition text-sm"
                          >
                            Mark as Paid
                          </button>
